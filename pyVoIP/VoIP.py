...
class VoIPCall():
  def __init__(self, phone, callstate, request, session_id, myIP, ms = None):
    # warnings.warn('DELETEME: voipcall init')
    ...
    # warnings.warn('DELETEME: voipcall HULLO')
    if callstate == CallState.RINGING:
      ...
      # warnings.warn('DELETEME: voipcall HULLO 2')
      ...
      for i in request.body['m']:
        ...
        # warnings.warn('DELETEME: voipcall HULLO 3')
        if e:
          raise RTP.RTPParseError("RTP Payload type {} not found.".format(str(pt)))
        # warnings.warn('DELETEME: voipcall HULLO 4')
        ...
        # warnings.warn('DELETEME: voipcall HULLO 5')
        port = self.phone.request_port()
        for ii in range(len(request.body['c'])):
          # warnings.warn('DELETEME: voipcall append rtcp client')
          try:
            ...
            self.RTPClients.append(RTP.RTPClient(codecs, self.myIP, port, request.body['c'][ii]['address'], i['port']+ii, request.body['a'].get('transmit_type', 'sendrecv'), dtmf=self.dtmfCallback)) #TODO: Check IPv4/IPv6
          except:
            traceback.print_exc()
        # warnings.warn('DELETEME: voipcall HULLO ringing end')
    elif callstate == CallState.DIALING:
      self.ms = ms
      for m in self.ms:
        self.port = m
        self.assignedPorts[m] = self.ms[m]
    # warnings.warn('DELETEME: voipcall really???')
  ...
  def renegotiate(self, request):
    ...
    message = self.sip.genAnswer(request, self.session_id, m, request.body['a'].get('transmit_type', 'sendrecv'))
  ...
  def answer(self):
    ...
    message = self.sip.genAnswer(self.request, self.session_id, m, self.request.body['a'].get('transmit_type', 'sendrecv'))
    self.sip.out.sendto(message.encode('utf8'), (self.phone.server, self.phone.port))
    self.state = CallState.ANSWERED
    # warnings.warn('DELETEME: VOIP request answered.')
  ...
  def answered(self, request):
    ...
    for i in request.body['m']:
      ...
      for ii in range(len(request.body['c'])):
        ...
        self.RTPClients.append(RTP.RTPClient(assoc, self.myIP, self.port, request.body['c'][ii]['address'], i['port']+ii, request.body['a'].get('transmit_type', 'sendrecv'), dtmf=self.dtmfCallback)) #TODO: Check IPv4/IPv6
  ...
  def hangup(self):
    # warnings.warn('DELETEME: Init Hangup')
    ...
    self.state = CallState.ENDED
    del self.phone.calls[self.request.headers['Call-ID']]
    # warnings.warn('DELETEME: End Hangup')
...
class VoIPPhone():
  def __init__(self, server, port, username, password, callCallback=None, myIP=None, sipPort=5060, rtpPortLow=10000, rtpPortHigh=20000):
    ...
    # warnings.warn('DELETEME: voip warn')
    ...
    if request.type == pyVoIP.SIP.SIPMessageType.MESSAGE:
      ...
      if request.method == "INVITE":
        ...
        if self.callCallback == None:
          ...
          # warnings.warn('DELETEME: We got a new call')
          debug("New call!")
          sess_id = None
          while sess_id == None:
          ...
          self.sip.out.sendto(message.encode('utf8'), (self.server, self.port))
          # warnings.warn('DELETEME: This is less weird')
          self.calls[call_id] = VoIPCall(self, CallState.RINGING, request, sess_id, self.myIP)
          # warnings.warn('DELETEME: This is weird')
          try:
            ...
            # logging.error('DELETEME: I just ate an exception')
            message = self.sip.genBusy(request)
            self.sip.out.sendto(message.encode('utf8'), (self.server, self.port))
            raise
