...
class SIPClient():
  ...
  def genRinging(self, request):
    # logging.error('DELETEME: Ringing some shit')
    tag = self.genTag()
    ...
    # logging.error('DELETEME: Mehhhhh')
  ...
  def tryingTimeoutCheck(self, response):
    ...
    while response.status == SIPStatus.TRYING:
      ...
      if (time.monotonic() - start_time) >= 30:
        raise debug("Timeout error for response, waited 30 seconds but SIPStatus is still TRYING")
    return response
